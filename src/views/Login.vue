<template>
  <div class="login-page-wrapper">
    <div class="login-box-wrapper" @click="changeImage(3)">
      <div class="login-box">
        <div>
        ...
        <van-tabs v-model="active" animated>
          <van-tab title="登录">
            <van-cell-group v-show="isUsernameLogin">
              <van-field v-model="login_user_name"
              required
              clearable
              label="用户名"
              @click.stop="changeImage(0)"
              placeholder="请输入用户名/手机号"/>
              <van-field v-model="login_user_pass"
              type="password"
              required
              label="密码"
            ...
            <van-cell-group v-show="!isUsernameLogin">
              <van-field
                ...
                label="手机号码"
                placeholder="请输入手机号"
                :error-message="checkPhoneNumber ? '' : '手机号格式不正确'"
              />
              <van-field
                center
                clearable
            ...
            <van-button type="info" size="large" style="margin-top:1rem" @click="login">登录</van-button>
              <div class="switchLogin" @click="switchLogin">
                {{!this.isUsernameLogin ? "账号密码登录" : "短信验证码登录"}}
              </div>
          </van-tab>
          <van-tab title="注册">
            <van-cell-group>
              <van-field
                v-model="regist_user_name"
                clearable
                maxlength="11"
                placeholder="请输入手机号"
                required
              />
              <van-field
                v-model="regist_user_pass"
                type="password"
                placeholder="请输入密码(不少于6位)"
                required
              />
              <van-field
                v-model="regist_user_pass2"
                type="password"
                placeholder="请再次输入密码(不少于6位)"
                required
              />
            </van-cell-group>
            <van-button type="info" size="large" style="margin-top:1rem" @click="regist">注册</van-button>
          </van-tab>
        </van-tabs>
...
export default {
  data () {
    return {
      ...
      regist_user_name: '13059228762',
      regist_user_pass: '591775790',
      regist_user_pass2: '591775790',

      shortMessageCaptcha: '', // 短信验证
      imgCaptcha: '', // 图片验证码
  ...
  computed: {
    ...
    // 2.发送验证码按钮显示
    captchaDisable () {
      if (this.login_phone.length > 10 && this.checkPhoneNumber) {
        return false
      } else {
        return true
      }
  ...
  methods: {
    ...
    // 2.登录
    login () {
      if (!this.isUsernameLogin) { // 2.1手机号+验证码登录
        if (this.captchaDisable) {
          Toast({
            message: '请输入正确的手机号',
            duration: 800
          })
        } else if (Number(this.shortMessageCaptcha) !== Number(this.shortMessageCaptchaResult)) {
          Toast({
            message: '请输入正确的验证码',
            duration: 800
          })
        } else {
          let ref = phoneCaptchaLogin(this.login_phone, this.shortMessageCaptcha)
          ref.then(response => {
        ...
        if (this.login_user_name.length < 1) {
          Toast({
            message: '请输入用户名',
            duration: 800
          })
        } else if (this.login_user_pass.length < 1) {
          Toast({
            message: '密码不能为空',
            duration: 800
          })
          ...
          let ref = phoneCaptchaLogin(this.login_user_name, this.login_user_pass)
          ref.then(response => {
            if (response.success_code === 200) {
              Toast({
                message: '登录成功',
    ...
    sendVerifyCode () {
      ...
      // 4.2获取短信验证码
      let result = getPhoneCaptcha(this.login_phone)
      console.log(result.success_code)
      if (result.success_code === 200) {
        this.shortMessageCaptchaResult = result.data.code
        Dialog.alert({
          title: '温馨提示',
    ...
    getCaptcha () {
      ...
      // 5.2captchaEle.setAttribute('src', 'http://demo.itlike.com/web/xlmc/api/captcha?time=' + new Date())
      this.$set(captchaEle, 'src', 'http://demo.itlike.com/web/xlmc/api/captcha?time=' + new Date())
    },
    // 6.注册
    regist () {
      // 6.1 用户名密码简单校验
      if (this.regist_user_name.length < 1) {
        Toast({
          message: '手机号不能为空',
          duration: 800
        })
      } else if (!/[1][3,4,5,6,7,8][0-9]{9}$/.test(this.regist_user_name)) {
        Toast({
          message: '手机号格式不正确',
          duration: 800
        })
      } else if (this.regist_user_pass.length < 0) {
        Toast({
          message: '请输入密码',
          duration: 800
        })
      } else if (this.regist_user_pass.length < 6) {
        Toast({
          message: '密码至少为6位哦!',
          duration: 800
        })
      } else if (this.regist_user_pass !== this.regist_user_pass2) {
        Toast({
          message: '两次输入密码不一致',
          duration: 800
        })
      } else {
        let ref = phoneCaptchaLogin(this.regist_user_name, this.regist_user_pass)
        ref.then(response => {
          if (response.data) {
            Toast({
              message: '注册成功,请登录',
