<template>
  <div class="order-time-wrapper">
    <van-popup v-model="showOrderTimePicker"
    round
    lock-scroll
    closeable
    @closed="closePopView"
...
export default {
  ...
  computed: {
    ...
    leftDateData () {
      ...
      let tomory = '明天' + Moment().add(1, 'days').format('MMMDo')
    ...
    sureCheckTime () {
      ...
      if (this.chooseDeliveryTime.length < 1) {
        if (this.currentIndex === 0) {
          this.chooseDeliveryTime = this.timeList.today[0]
        } else {
          this.chooseDeliveryTime = this.timeList.tomorrow[0]
        }
      }
      this.$emit('changeData', false, date + this.chooseDeliveryTime)
    },
